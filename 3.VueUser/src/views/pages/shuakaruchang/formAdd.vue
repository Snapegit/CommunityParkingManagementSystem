...
<template>
	<div class="app-contain" :style='{"minHeight":"100vh","padding":"0 18%","margin":"20px auto 60px","color":"#666","background":"#fff","width":"100%","fontSize":"14px","position":"relative","height":"100%"}'>
		...
		<el-form ref="formRef" :model="form" class="add_form" label-width="120px" :rules="rules">
			<el-row>
				<el-col :span="24">
					<el-form-item label="车位号" prop="cheweihao">
						<el-input class="list_inp" v-model="form.cheweihao" placeholder="车位号"
							 type="text" 							:readonly="!isAdd||disabledForm.cheweihao?true:false" />
					</el-form-item>
				</el-col>
				...
				<el-col :span="24">
					<el-form-item label="车牌号码" prop="chepaihaoma">
						<el-select
							class="list_sel"
							:disabled="!isAdd||disabledForm.chepaihaoma?true:false"
							v-model="form.chepaihaoma" 
							placeholder="请选择车牌号码"
							style="width:100%;"
							@change="chepaihaomaChange">
							<el-option v-for="(item,index) in chepaihaomaLists" :label="item" :value="item">
							</el-option>
						</el-select>
					</el-form-item>
				</el-col>
...
<script setup>
	...
	const init = (formId=null,formType='add',formNames='',row=null,table=null,statusColumnName=null,tips=null,statusColumnValue=null) => {
		...
		if(formType == 'add'){
			...
			for(let x in row){
				if(x=='cheweihao'){
					form.value.cheweihao = row[x];
					disabledForm.value.cheweihao = true;
					continue;
				}
	...
	//车牌号码回调
	const chepaihaomaChange=()=>{
		context?.$http({
			url: `follow/cheliangxinxi/chepaihaoma?columnValue=` + form.value.chepaihaoma,
			method: 'get'
		}).then(res=>{
			//带出车辆名称字段
			if(res.data.data.cheliangmingcheng){
				form.value.cheliangmingcheng = res.data.data.cheliangmingcheng
			}
		})
	}
	//提交
