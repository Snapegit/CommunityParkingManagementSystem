			...
			<div class="search_view">
				...
				<div class="search_box">
					<el-select
						class="search_sel"
						clearable
						v-model="searchQuery.cheliangleixing" 
						placeholder="车辆类型"
						>
						<el-option v-for="item in cheliangleixingLists" :label="item" :value="item"></el-option>
					</el-select>
				</div>
			</div>
			...
			<div class="data_box">
				<div class="data_view">
					<el-table v-loading="listLoading" class="data_table" :data="list" border :row-style="{'cursor':'pointer'}"
						...
						<el-table-column label="车辆类型" :resizable='true' align="left" header-align="left">
							<template #default="scope">
...
<script setup>
	...
	//下拉列表
	const cheliangleixingLists = ref([])
	const getcheliangleixingLists = () => {
		context?.$http({
			url: 'option/cheliangleixing/cheliangleixing',
			method:'get'
		}).then(res=>{
			cheliangleixingLists.value = res.data.data
		})
	}
	getcheliangleixingLists()
	const searchClick = () => {
	...
	const getList = () => {
		...
		if (categoryIndex.value != -1) {
			params.cheliangleixing = categoryList.value[categoryIndex.value]
		}
