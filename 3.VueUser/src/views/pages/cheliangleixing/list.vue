	...
	<div class="app-contain" :style='{"padding":"0 18%","margin":"0","overflow":"hidden","color":"#666","alignItems":"flex-start","flexWrap":"wrap","background":"#fff","display":"flex","width":"100%","fontSize":"14px","justifyContent":"space-between"}'>
		...
		<el-form :inline="true" :model="searchQuery" class="list_search">
			...
			<div class="search_btn_view">
				...
				<el-button class="add_btn" type="success" v-if="btnAuth('cheliangleixing','新增')" @click="addClick">新增</el-button>
			</div>
		</el-form>
...
<script setup>
	...
	const addClick = () => {
		router.push('/index/cheliangleixingAdd')
	}
	//判断是否从个人中心跳转
	...
	const tableDetailClick = (row) => {
		router.push('cheliangleixingDetail?id=' + row.id + (centerType.value?'&&centerType=1':''))
	}
	//下载文件
	const download = (file) =>{
		if(!file){
